...
class Score_board_model extends CI_Model{
 ...
 function getJobRes($jobId){
 	$this->db->where(array($this->pref('job.ID')=>$jobId));
 ...
 function get_score($scID,$userId){
 	$data=array(
 ...
 function get_job_res_from_tId($tId=0){
 	$this->db->where($this->pref('targets.ID'),$tId);
 	$this->db->where('LEFT( url, 4 )=' , 'file');
 								
 	$this->db->select($this->pref('job_result.url AS url'));
 	$this->db->join($this->pref('job'),$this->pref('job.target_id').'='.$this->pref('targets.ID'),'left');
 	$this->db->join($this->pref('schedule'),$this->pref('schedule.job_id').'='.$this->pref('job.ID'),'left');
 	$this->db->join($this->pref('job_result'),$this->pref('job_result.schedule_id').'='.$this->pref('schedule.ID'),'left');
 	$query=$this->db->get($this->pref('targets'));
 	return $query->result();
 }
 ...
 function get_job_res_by_id($id,$userid){
 	$this->db->where(array($this->pref('job_result.ID')=>$id));
 	$this->db->where(array($this->pref('targets.user_id')=>$userid));
 	$this->db->select(array(
 		$this->pref('job_result.ID as resId'),
 		$this->pref('job_result.url as url')
 		));
 ...
 function delete_result($id,$userid){
 	$data=$this->get_job_res_by_id($id,$userid);
 	if($data->num_rows() >= 1) $this->delete_row($this->pref('job_result'),array('ID'=>$id));
 	else return false;
 }
 ...
 function select_job_res_file_only($job_id,$user_id){
 	$this->db->where(array($this->pref('job.ID')=>$job_id));
 	$this->db->where(array($this->pref('targets.user_id')=>$user_id));
 	$this->db->where(array('LEFT('.$this->pref('job_result.url').',4)'=>'file'));
 ...
 function save_post_period($target_id=0,$user_id=0){
			...
			if(abs($difrnc_start) <= abs($max_value_start)){
				...
				for ($date=0;$date>=$difrnc_start+1;$date--){ //$difrnc_start-1
					...
					foreach($q_job as $r){
						...
						$data_file=$this->get_job_res_file_by_jobId($job_id);
						foreach($data_file as $dt){
							$url=$dt->url;
 ...
 function get_job_res_file_by_jobId($job_id){
 	$this->db->where($this->pref('schedule.job_id'),$job_id);
 	$this->db->where('LEFT('.$this->pref('job_result.url').',4)','file');
 	$this->db->select(array($this->pref('job_result.ID'),$this->pref('job_result.url')));
 	$this->db->join($this->pref('schedule'),$this->pref('schedule.ID').'='.$this->pref('job_result.schedule_id'),'left');
	$q=$this->db->get($this->pref('job_result'));
 	$data=$q->result();
 	return $data;
 }
 ...
 function get_all_users(){
 	...
 	$this->db->select(array('ID','nama_lengkap'));
 ...
 function filter_targets(){
 	if($_POST){
 		$user_id=$this->input->post('userId');
 			if(!empty($user_id))$this->db->where('pengguna.ID',$user_id);
 		$year=$this->input->post('cmb_tahun');
 			if(!empty($year))$this->db->where('LEFT('.$this->pref('targets.period_start').',4)',$year);
 		
 	}
 		return $this->get_all_targets();
 }
